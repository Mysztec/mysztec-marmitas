...
/** Ids dos funcionarios travados por excesso de tentativas. Somente admin. */
export async function funcionariosBloqueados() {
  const linhas = await db.rpc('locked_employees');
  return new Set((linhas || []).map(l => (typeof l === 'string' ? l : l.locked_employees)));
}
